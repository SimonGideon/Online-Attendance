  ...
  form do |lecturer_unit|
    ...
    concat(content_tag(:ol) do # Added class list-unstyled for removing default list styles
      lecturer_unit.students_courses.each do |students_course|
        ...
        concat(content_tag(:li, class: "d-flex justify-content-between align-items-center mb-2 ") do # Added classes for flex layout and margin-bottom
          concat(content_tag(:span, student.name)) # Assuming the student has a name attribute
          concat(button_to("Delete", "/students/#{student.id}/students_courses/#{students_course.id}", method: :delete, data: { confirm: "Are you sure you want to delete this student?" }, class: "btn btn-danger"))
        end)
      end
    end)
  end
